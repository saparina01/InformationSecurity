class AES:
    ...
    def aes_decrypt(self, ciphertext_list, RoundKeys):
        ...
        State = self.ShiftRows_Inv(State)
        print("SR_inv后：" + str(State))
        State = self.SubBytes_Inv(State)
        print("SB_inv后：" + str(State))
        State = self.AddRoundKey(State, RoundKeys, 0)
        print("ARK_inv后：" + str(State))
        return State

if __name__ == '__main__':

    aes = AES()
    #128bit（二进制）-32位(16进制)-16字节
    key = 0x000102030405060708090a0b0c0d0e0f
    RoundKeys = aes.round_key_generator(key)


    plaintext = 0x001929364a5f63718099ac
    ...
    result = aes.aes_decrypt(ciphertext, RoundKeys)
    print('plaintext = ' + hex(aes._16bytes2num(result)))
